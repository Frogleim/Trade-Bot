...
scalping_file_path = f"{files_dir}/files/scalping_data.csv"
...
def insert_data(entry_price, close_price, profit, entry_price_diff=0.0, open_time=None, close_time=None):
    try:
        ...
        df = pd.DataFrame(
            columns=["entry price", "close price", "profit", "entry price difference", 'open_time', 'close_time'])
    new_data = {
...
def insert_scalping_data(entry_price, close_price, profit, entry_price_diff=0.0, open_time=None, close_time=None):
    try:
        df = pd.read_csv(scalping_file_path)
    except FileNotFoundError:
        # If the file doesn't exist, create a new DataFrame
        df = pd.DataFrame(
            columns=["entry price", "close price", "profit", "entry price difference", 'open_time', 'close_time'])
