...
def calculate_bollinger_bands(interval, length, num_std_dev):
    ...
    df['low'] = [float(kline[3]) for kline in klines]
    df['high'] = [float(kline[2]) for kline in klines]
    ...
    # Get the minimum close price of the last candle
    previous_candle_low_price = df['low'].iloc[-2]
    previous_candle_high_price = df['high'].iloc[-2]


    return df, previous_candle_low_price, previous_candle_high_price
...
def trade():
    global closed
    num_periods = 10
    data, low_price, high_price = calculate_bollinger_bands(interval, length, config.num_std_dev)
