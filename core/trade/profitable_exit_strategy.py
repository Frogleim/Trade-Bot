...
def trade():
    ...
    if btc_price_change and crypto_current <= opened_price:
        ...
        try:
            entry_price = crypto_ticker.place_buy_order(price=opened_price, quantity=config.position_size, symbol=config.trading_pair)
        ...
        logging.info(body)
        while True:
            res = pnl_long(opened_price=entry_price, signal=signal_price)
            if res == 'Profit':
                try:
        ...
        logging.info(f'Profit checkpoint list: {profit_checkpoint_list} --- Current checkpoint: {current_checkpoint}')
        try:
            entry_price = crypto_ticker.place_sell_order(price=opened_price, quantity=config.position_size, symbol=config.trading_pair)
        except Exception as e:
            logging.error(e)
            entry_price = crypto_ticker.place_sell_order(price=opened_price, quantity=config.position_size, symbol=config.trading_pair)
        ...
        logging.info(body)
        while True:
            res = pnl_short(opened_price=entry_price, signal=signal_price)
            if res == 'Profit':
                try:
...
def check_price_changes():
    global checking_price, recent_signal, price_history, SMA, trend
